...
class DashboardTest:
    ...
    def test_log_analyzer(self, run_dir):
        ...
        try:
            ...
            if not logs_dir.exists():
                self.add_result("LOG_ANALYZER", False, f"Logs directory not found: {logs_dir}")
                return False
            
            # Initialize log analyzer with the logs directory
            analyzer = LogAnalyzer(str(logs_dir))
            
            # Find and parse the latest log file
            log_file = analyzer.find_latest_log()
            if not log_file:
                self.add_result("LOG_ANALYZER", False, "No log files found in logs directory")
                return False
            ...
            # The analysis data is now in analyzer.data
            analysis = analyzer.data
            
            # Basic validation of analysis results
            if not analysis:
                self.add_result("LOG_ANALYZER", False, "No analysis data generated")
                return False
            
            self.add_result("LOG_ANALYZER", True, f"Successfully analyzed: {log_file}")
            return analysis
    ...
    def test_dashboard_generation(self, run_dir, analysis):
        ...
        try:
            ...
            # Check for basic HTML structure (less strict)
            ...
            if len(content) < 100:
                self.add_result("DASHBOARD_GENERATION", False, "Dashboard content too short")
                return False
            
            self.add_result("DASHBOARD_GENERATION", True, f"Generated dashboard: {dashboard_file.name} ({len(content)} chars)")
            return True
