            ...
            content += `
                <div class="source-concern-item mb-2 p-2 border rounded" style="border-left: 4px solid #9C27B0 !important;">
                    <div class="concern-header mb-2">
                        <div class="d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 text-danger fw-bold" style="font-size: 0.9rem;">Concern #${issueNumber}</h6>
...
`;
    ...
    static generateOverlappingRangesContent(concern) {
        ...
        // Handle wildcard multiple bounds case (as generated by codebase)
        if (overlapType === 'wildcard_multiple_bounds') {
            ...
            if (Array.isArray(concern.sourceValue)) {
                propertyDisplays = concern.sourceValue.map(item => {
                    ...
                    return `<div class="row mt-1"><div class="col-12"><code class="bg-light px-1 rounded" style="font-size: 0.8rem;">"${source}": { ${properties.join(', ')} }</code></div></div>`;
                });
            }
            
            const propertyDisplay = propertyDisplays.length > 0 
                ? propertyDisplays.join('')
                : '<div class="row mt-1"><div class="col-12">multiple wildcard entries with different bounds</div></div>';
            
            // Generate dynamic overlap explanation for wildcard case (matching dashboard logic)
            let overlapNote = '';
            if (Array.isArray(concern.sourceValue) && concern.sourceValue.length > 1) {
                ...
                if (allWildcard && uniqueBounds > 1) {
                    overlapNote = `<div class="row mt-2"><div class="col-12"><em class="text-muted" style="font-size: 0.85rem;">All ${rangeCount} wildcard ranges overlap because they start from "*" (beginning of time) but have ${uniqueBounds} different upper bounds: ${branches.join(', ')}.</em></div></div>`;
                } else if (allWildcard) {
                    overlapNote = `<div class="row mt-2"><div class="col-12"><em class="text-muted" style="font-size: 0.85rem;">All ${rangeCount} wildcard ranges overlap because they start from "*" with different boundary types.</em></div></div>`;
                } else {
                    overlapNote = `<div class="row mt-2"><div class="col-12"><em class="text-muted" style="font-size: 0.85rem;">All ${rangeCount} ranges overlap due to conflicting version patterns.</em></div></div>`;
                }
            }
            ...
            return `
                <div class="concern-content">
                    <div class="problem-description mb-2">
                        <strong class="text-danger">Problem:</strong>
                        <p class="mb-2">Multiple wildcard patterns create ambiguous range definitions with different upper bounds.</p>
                    </div>
                    <div class="problematic-data mb-2">
                        <strong class="text-warning">Data:</strong>
                        <div class="data-display mt-1">
                            <div class="row">
                                ...
                                <div class="col-8">
                                    <span class="badge bg-warning text-white" style="font-size: 0.7rem;">
                                        WILDCARD MULTIPLE BOUNDS
                                    </span>
                                </div>
                            </div>
                            ${propertyDisplay}
                    ...
                    <div class="resolution-guidance">
                        ...
                        <div class="mb-0 text-muted" style="font-size: 0.85rem;">
                            Consolidate wildcard patterns or use explicit version ranges to eliminate ambiguity.
                        </div>
                    </div>
                </div>
            `;
        ...
        if (overlapType === 'identical_ranges') {
            ...
            humanReadableDescription = 'Range containment overlap detected';
        } else if (overlapType === 'range2_contains_range1') {
            humanReadableDescription = 'Range containment overlap detected';
        }
        
        // Simple overlap indication for standard range cases (matching dashboard logic)
        const overlapSources = `${range1Source} ↔ ${range2Source}`;
                        ...
                        <div class="row">
                            ...
                            <div class="col-8">
                                <span class="badge ${overlapType === 'identical_ranges' ? 'bg-danger' : overlapType.includes('contains') ? 'bg-warning' : 'bg-info'} text-white" style="font-size: 0.7rem;">
                                    ${humanReadableDescription.toUpperCase()}
                                </span>
                            </div>
                        </div>
                        <div class="row mt-1">
                            <div class="col-4"><strong>Range 1:</strong></div>
                            <div class="col-8"><code class="bg-light px-1 rounded" style="font-size: 0.8rem;">${range1Source}: ${range1}</code></div>
                        </div>
                        <div class="row mt-1">
                            <div class="col-4"><strong>Range 2:</strong></div>
                            <div class="col-8"><code class="bg-light px-1 rounded" style="font-size: 0.8rem;">${range2Source}: ${range2}</code></div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-12"><em class="text-muted" style="font-size: 0.85rem;">Overlapping ranges: ${overlapSources}</em></div>
                        </div>
                    </div>
                </div>
                <div class="resolution-guidance">
