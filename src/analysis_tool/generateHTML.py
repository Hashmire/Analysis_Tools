...
def getCPEJsonScript() -> str:
    ...
    js_content += json_settings_injection
...
def update_cpeQueryHTML_column(dataframe, nvdSourceData):
    ...
    for index, row in result_df.iterrows():
        ...
        store_json_settings_html(table_id)
...
def create_json_generation_settings_html(table_id):
    """Creates the JSON generation settings form HTML for a specific table
    
    Args:
        table_id: The table ID (e.g., "matchesTable_0")
    """
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <h6 class="text-muted mt-3 mb-2">Processing Mode</h6>
                        <div class="form-check mb-1">
                            <input class="form-check-input row-setting" type="radio" 
                                   name="processingMode_{table_id}" 
                                   id="modeBasic_{table_id}" 
                                   value="basic"
                                   data-setting="processingMode"
                                   data-table-id="{table_id}">
                            <label class="form-check-label" for="modeBasic_{table_id}">
                                <small>Basic <span class="text-muted">(simple matching)</span></small>
                            </label>
                        </div>
                        <div class="form-check mb-1">
                            <input class="form-check-input row-setting" type="radio" 
                                   name="processingMode_{table_id}" 
                                   id="modeEnhanced_{table_id}" 
                                   value="enhanced"
                                   data-setting="processingMode"
                                   data-table-id="{table_id}" 
                                   checked>
                            <label class="form-check-label" for="modeEnhanced_{table_id}">
                                <small>Enhanced <span class="text-muted">(intelligent ranges)</span></small>
                            </label>
                        </div>
                    </div>
                </div>
...
def store_json_settings_html(table_id):
    """Store the JSON settings HTML for a table to be accessed by JavaScript"""
    JSON_SETTINGS_HTML[table_id] = create_json_generation_settings_html(table_id)
