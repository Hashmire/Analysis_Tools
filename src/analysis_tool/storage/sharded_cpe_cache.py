#!/usr/bin/env python3
"""
Sharded CPE Cache Implementation

Hash-based sharded cache with lazy loading and run-boundary eviction for improved
memory management and scalability. Addresses 910 MB+ monolithic cache file issues.
...
- Compact JSON storage (no OPT_INDENT_2 formatting overhead)
- Auto-save every N entries for data safety
- Backward-compatible API with existing CPECache
"""
...
class ShardedCPECache:
    ...
    Replaces monolithic CPECache with sharded architecture for improved
    ...
    def __init__(self, config: Dict[str, Any], num_shards: int = 16):
        ...
        self.cache_dir = project_root / "cache" / "cpe_shards"
        ...
        # Session statistics (same as monolithic cache)
        self.session_stats = {
            'hits': 0,
            'misses': 0,
            'expired': 0,
    ...
    def _get_shard_index(self, cpe_string: str) -> int:
        ...
        Args:
            cpe_string: CPE match string (e.g., "cpe:2.3:a:vendor:product")
    ...
    def _get_shard_filename(self, shard_index: int) -> str:
        ...
        Args:
            shard_index: Shard index (0 to num_shards-1)
            
        Returns:
            Shard filename (e.g., "cpe_cache_shard_00.json")
        """
    ...
    def _load_shard(self, shard_index: int) -> Dict[str, Any]:
        ...
        Args:
            shard_index: Shard index to load
            
        Returns:
            Shard cache data dict
        """
    ...
    def _save_shard(self, shard_index: int) -> None:
        ...
        Args:
            shard_index: Shard index to save
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get cache statistics (compatible with monolithic cache API)."""
        return {
            'total_entries': self._get_total_entries(),
