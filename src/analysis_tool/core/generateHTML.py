...
def update_cpeQueryHTML_column(dataframe):
    ...
    # Note: Overlapping ranges detection is now handled entirely by the badge modal system
    # during source data concerns processing. The consolidated cross-platform analysis
    # is performed by detect_cross_affected_entry_overlaps() and detect_comprehensive_range_overlaps()
    logger.debug(f"Overlapping ranges detection delegated to badge modal system for {len(result_df)} platform entries", group="PAGE_GEN")
