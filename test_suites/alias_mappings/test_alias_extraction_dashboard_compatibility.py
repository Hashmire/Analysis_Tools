...
class AliasExtractionDashboardCompatibilityTestSuite:
    ...
    def find_alias_extraction_report(self) -> Optional[Path]:
        ...
        try:
            # Check if running under unified test runner (consolidated environment)
            if os.environ.get('UNIFIED_TEST_RUNNER') or os.environ.get('CONSOLIDATED_TEST_RUN'):
                # Look in consolidated test run logs directory
                consolidated_path = os.environ.get('CONSOLIDATED_TEST_RUN_PATH')
                if consolidated_path:
                    consolidated_logs = Path(consolidated_path) / "logs"
                    if consolidated_logs.exists():
                        # Look for alias extraction runs in consolidated logs
                        alias_runs = [d for d in consolidated_logs.iterdir() 
                                    if d.is_dir() and "alias" in d.name.lower()]
                        alias_runs.sort(key=lambda x: x.stat().st_mtime, reverse=True)
                        
                        for alias_run in alias_runs:
                            report_file = alias_run / "logs" / "aliasExtractionReport.json"
            ...
            # Fallback to standard directory search
