...
TEST STRATEGY:
    - Use actual tool execution (python -m src.analysis_tool.core.cve_processor)
...
ENTRY POINT COVERAGE:
    - Direct: python -m src.analysis_tool.core.cve_processor --cve ...
    - Note: generate_dataset.py calls cve_processor.main() internally, so it uses
...
class MultiCVEStateIsolationTestSuite:
    ...
    def run_tool_with_cves(self, cve_list: List[str]):
        """
        Run the actual CVE processor with specified CVEs
